 ...
 */
export const applySlateOperations = (doc, docId, slateOperations, clientId) => {
    ...
    return {changes: Automerge.getChanges(currentDoc, docNew), docNew}
}
    ...
    operations.forEach(op => {
        ...
        switch (op.type) {
            ...
            case "insert_text":
                ...
                console.log(`insert_text:offset(${offset}), text(${text}), alltext(${currentNode.leaves[0].text.join()})`)
                let tmpLen = text.length;
                for(let i = 0; i < tmpLen; i++){
                    currentNode.leaves[0].text.insertAt(offset+i, text[i]);
                }
                break;
            case "remove_text":
                path.forEach(el => {
